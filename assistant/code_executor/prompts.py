...
`render_untrusted_block` is re-exported here for the ten call sites that
import it from this module; it now lives in `core/fence.py`, below every layer
that builds a prompt. The reason it is in one place at all is unchanged: the
planner's pseudo-tools and `read_screen` render the same block, and three
copies of a security control drift.
"""
...
#
# Moved to `core/fence.py` when the Context Builder needed it: `core/` is the
# bottom layer, and §12.1's C2 asks for one fence applied at the boundary
# rather than one per caller. Re-exported here because ten call sites across
# `actions/` import it from this module, and a move that rewrites ten imports
# to gain nothing is churn.
from ..core.fence import (  # noqa: F401
    _fence_nonce, _neutralise, _UNTRUSTED_NOTICE, render_untrusted_block,
)
